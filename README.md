...
一款防止网页视频暂停并拦截鼠标移出事件的 Chrome 扩展。附有专门用于自动化播放学习通视频课程（刷课）的程序 `Auto.py`。
...
- **自动翻页并检测任务完成状态**。
- **暂不包含辅助答题功能，会跳过答题页面**
- **！注意事项！** ：该功能目前尚不完全，请在长时间运行（比如夜间运行）前进行简单测试，如果有点击错误（如点击了题目页面的提交），直接关闭Chrome页面或关闭程序即可。针对视频内答题部分尚在开发中，敬请期待。

## 插件安装步骤
1. 打开 Chrome 浏览器，前往 `chrome://extensions/`。
2. 在右上角启用“开发者模式”（切换开关）。
3. 点击“加载已解压的扩展”。
4. **注：安装插件前，必须先把loc文件与Auto.py移除** -> 选择包含 `manifest.json` 的文件夹。
5. 扩展安装完成，即刻生效！
...
## 插件的风险与注意事项
- **使用范围**：该插件默认适用于所有网站，如果只针对某个固定网站可以修改`manifest.json`中相关代码。
